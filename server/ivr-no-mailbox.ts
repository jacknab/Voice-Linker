...
function playTimeRemaining(
  ...
  // Try composite pre-recorded file first (single natural utterance, if generated).
  ...
  // Build from component audio files — avoids TTS for any minute value.
  // Pattern: phrase_you_have + num_N + phrase_(X)_of_pbtr
  if (totalMinutes >= 1440) {
    ...
    if (hours === 0) {
      ...
      playPrompt(twiml, req, days === 1 ? "phrase_day.mp3" : "phrase_days.mp3", days === 1 ? "day" : "days");
    ...
    if (mins === 0) {
      ...
      playPrompt(twiml, req, hours === 1 ? "phrase_hour.mp3" : "phrase_hours.mp3", hours === 1 ? "hour" : "hours");
      playPrompt(twiml, req, "phrase_and.mp3", "and");
...
// Mailboxes and personal ads are permanently disabled in this IVR variant.
const MAILBOX_ENABLED = false;
...
export async function registerVoiceRoutes(app: Express): Promise<void> {
  ...
  async function getOrCreateUser(phoneNumber: string, callSid?: string) {
    if (phoneNumber && !isRealPhoneNumber(phoneNumber)) {
      if (callSid) {
  ...
  app.post("/voice", async (req, res) => {
    ...
    if (isAnonymous) {
      await getSiteSettingsCached().catch(() => {});
      storage.logCall(callSid, "anonymous", req.body?.To || null, null).catch(() => {});
  ...
  app.post("/voice/entry-check", async (req, res) => {
    ...
    try {
      ...
      if (isFreeModeActive(nmEntrySettings)) {
        // Free mode — no balance checks; auto-activate new callers and go straight in
        if (!user.membershipTier) {
          await storage.updateUserMembership(user.id, {
            membershipTier: "free_trial",
            remainingSeconds: nmEntrySettings.freeTrialMinutes * 60,
          });
          await storage.getOrCreateMailbox(user.id);
          console.log(`[entry-check] Auto-activated free trial (free mode) — ${nmEntrySettings.freeTrialMinutes} min for userId=${user.id}`);
        }
        const siteConf = await getSiteSettingsCached();
        twiml.redirect(siteConf.siteCategory === "MW" ? "/voice/mw-main-menu" : "/voice/main-menu");
      } else if (!user.membershipTier) {
        // Brand new caller — auto-activate free trial and announce time
        await storage.updateUserMembership(user.id, {
          membershipTier: "free_trial",
          remainingSeconds: nmEntrySettings.freeTrialMinutes * 60,
        });
        await storage.getOrCreateMailbox(user.id);
        console.log(`[entry-check] Auto-activated free trial — ${nmEntrySettings.freeTrialMinutes} min for userId=${user.id}`);
        playTimeRemaining(twiml, req, nmEntrySettings.freeTrialMinutes);
        playPrompt(twiml, req, "free_trial_terms.mp3",
          "Your free trial will expire in seven days and it must be used from this phone number.");
        callTimeAnnounced.add(callSid);
        const siteConf = await getSiteSettingsCached();
  ...
  app.post("/voice/handle-mailbox-lookup", async (req, res) => {
    ...
    try {
      ...
      // Rejection gate — intercept before the send-message menu plays
      if (fromNumber) {
        const lookupUser = await getOrCreateUser(fromNumber, callSid);
  ...
  app.post("/voice/cs-billing-info", async (req, res) => {
    ...
    try {
      ...
      if (user.membershipTier === "free_trial") {
        planContext = `You are on a free trial with ${settings.freeTrialMinutes} minutes. `;
      } else if (user.membershipTier) {
        planContext = "You are on a paid membership. ";
      } else {
        planContext = "You do not have an active membership. ";
      }
    } catch {}
  ...
  app.post("/voice/cs-leave-message", async (req, res) => {
    ...
    twiml.record({ action: "/voice/cs-save-message", finishOnKey: "any", maxLength: 180, timeout: 5 });
    twiml.redirect("/voice/customer-service");
    res.type("text/xml");
    res.send(twiml.toString());
  });
  ...
  app.post("/voice/review-greeting", async (req, res) => {
    ...
    try {
      ...
      if (fromNumber) {
        const reviewUser = await getOrCreateUser(fromNumber, callSid);
    ...
    try {
      ...
      if (unreadMessage) {
        ...
        if (state.queue.length === 0) {
          ...
          //
          // Profiles are fetched per-region (home + any linked regions opted into),
          // matching the exact scope used when the queue was originally built.
          // The same results are reused for new-caller detection, so there are no
          // extra DB calls compared to the previous approach.
          ...
          if (regionId) {
            ...
            if (newLocalCaller) {
              ...
              state.queue.splice(insertAt, 0, {
                ...
                isNearby: nearbySet.has(newLocalCaller.userId),
              });
          ...
          // (isNewCallerAlert=true). The flag is set exactly when a caller joins after
          // queue init — no probabilistic budget or proximity math needed.
  ...
  // Played when a caller exhausts their region's queue.
  ...
  app.post("/voice/handle-nearby-callers", async (req, res) => {
    ...
    try {
      ...
      if (digit === "1" && state && chosenRegionId) {
        // Load profiles from the first linked region (primary offer)
        const user = await getOrCreateUser(fromNumber, callSid);
        const callerZip = user.zipCodeId ? await storage.getZipEntryById(user.zipCodeId) : null;
        ...
        const nearbySet = new Set<string>(
          callerLat != null && callerLon != null
            ? await storage.getNearbyProfileUserIds(user.id, chosenRegionId, callerLat, callerLon, 80)
            : []
        );
        ...
        if (linkedProfiles.length > 0) {
          ...
          console.log(`[voice] handle-nearby-callers: loaded ${linkedProfiles.length} profiles from "${chosenRegionName}" for ${callSid}`);
          playPrompt(twiml, req, "nearby_callers_intro.mp3", "Now playing callers from nearby. Enjoy!");
        } else {
          // No callers in linked region — restart local
          state.index = 0;
          state.linkedRegionLoaded = true;
          state.hasWrapped = false;
        ...
        // Star, timeout, or any other input → reset local queue
        if (state) {
          state.index              = 0;
  ...
  app.post("/voice/handle-message-menu", async (req, res) => {
    ...
    try {
      ...
      if (digit === "1") {
        ...
        console.log(`[live-connect] Press 1 (message menu) — from=${fromNumber} callSid=${callSid} senderId=${senderId}`);
              ...
              if (!targetActiveCall) {
                ...
                if (!liveConnectFreeMode && (!targetUser || (targetUser.remainingSeconds ?? 0) < 300)) {
                  ...
                  if (isBlocked) {
                    ...
                    console.log(`[live-connect] ALL CHECKS PASSED (msg-menu) — creating invite from userId=${user.id} → senderId=${senderId}`);
                    await storage.markMessageRead(msgId);
  ...
  app.post("/voice/handle-profile-menu", async (req, res) => {
    ...
    try {
      ...
      if (digit === "1") {
        ...
        console.log(`[live-connect] Press 3 received — from=${fromNumber} callSid=${callSid} profileUserId=${profileUserId}`);
        ...
        if (!fromNumber || !profileUserId || !callSid) {
          ...
          console.log(`[live-connect] ALL CHECKS PASSED — prompting for invite recording: userId=${user.id} → targetUserId=${profileUserId} isVirtual=${isVirtualTarget}`);

          // Prompt the caller to record a brief invite message
        ...
        const callSid5 = req.body?.CallSid as string;
        const state5   = await getBrowseState(callSid5);

        if (state5?.browsingLinked) {
          // ── Return to local guys from linked-region browsing ──────────────
          state5.browsingLinked          = false;
          state5.browsingLinkedRegionId  = null;
          state5.browsingLinkedRegionName = null;
          state5.linkedRegionLoaded      = false;
          state5.index                   = 0;
          state5.hasWrapped              = false;
          await setBrowseState(callSid5, state5);
          ...
          if (state5 && state5.lastPlayedIndex !== null) {
            if (state5.lastPlayedIndex > 0) {
              state5.index = state5.lastPlayedIndex - 1;
              console.log(`[voice] handle-profile-menu: press 5 → rewinding to index ${state5.index} for callSid=${callSid5}`);
              await setBrowseState(callSid5, state5);
              twiml.redirect("/voice/browse-profiles");
            } else if (state5.hasWrapped && state5.queue.length > 1) {
              state5.index = state5.queue.length - 1;
              console.log(`[voice] handle-profile-menu: press 5 → wrap-back to index ${state5.index} for callSid=${callSid5}`);
              await setBrowseState(callSid5, state5);
              twiml.redirect("/voice/browse-profiles");
            } else {
              playPrompt(twiml, req, "live_connect_left_line.mp3", "That caller has left the line.");
              state5.index = state5.lastPlayedIndex;
              await setBrowseState(callSid5, state5);
              twiml.redirect("/voice/browse-profiles");
            }
          } else {
  ...
  // ─── 8a-pre. Live Connect: Receive Invite Recording ─────────────────────
  // Caller A lands here after Twilio records their brief invite message.
  // Plays it back so they can hear how it sounds, then ask them to press 1 to send.
  app.post("/voice/live-connect-record-invite-done", async (req, res) => {
    ...
    try {
      ...
      const confirmGather = twiml.gather({
        numDigits: 1,
        action: `/voice/live-connect-confirm-invite?targetUserId=${encodeURIComponent(targetUserId)}&callSid=${encodeURIComponent(callSid)}&recordingUrl=${encodeURIComponent(recordingUrl ?? "")}`,
        timeout: 20,
      });
      if (recordingUrl) {
        safePlayRecording(confirmGather, recordingUrl, req, "");
      }
      playPrompt(confirmGather, req, "live_connect_confirm_invite.mp3",
        "Here's how it sounds. Press 1 to send your invitation to this caller at any time.");
  ...
  // ─── 8a-pre2. Live Connect: Confirm Invite Before Sending ────────────────
  // Caller A pressed 1 after hearing their recording played back. Create the
  // invite now and enter the wait loop.
  app.post("/voice/live-connect-confirm-invite", async (req, res) => {
    const twiml = new VoiceResponse();
    const targetUserId     = req.query.targetUserId as string;
    const initiatorCallSid = req.query.callSid as string;
    const recordingUrl     = req.query.recordingUrl as string | undefined;
    const fromNumber       = req.body?.From    as string;
    const callSid          = req.body?.CallSid as string;
    const digit            = req.body?.Digits  as string;

    try {
      if (!targetUserId || !fromNumber) {
        playPrompt(twiml, req, "error_generic.mp3", "An error occurred. Returning to profiles.");
        twiml.redirect("/voice/browse-profiles");
        res.type("text/xml");
        return res.send(twiml.toString());
  ...
  // Caller A lands here after confirming their invite. Loops a ringing clip
  // for up to 60 seconds (6 iterations of ~10 s each).
  // Pressing # at any time cancels the request.
  ...
  app.post("/voice/live-connect-wait", async (req, res) => {
    ...
    try {
      ...
      if (digit === "#") {
        const staleInvite = pendingLiveInvites.get(targetUserId);
        if (staleInvite) pendingLiveInvites.delete(targetUserId);
        playPrompt(twiml, req, "live_connect_cancelled.mp3",
          "Your connection request has been cancelled. Returning to profiles.");
        twiml.redirect("/voice/browse-profiles");
        res.type("text/xml");
        return res.send(twiml.toString());
      }
      ...
      if (!invite) {
        ...
        twiml.redirect("/voice/browse-profiles");
      } else if (invite.status === "declined" || ringCount >= MAX_RING_LOOPS || Date.now() - invite.createdAt > LIVE_INVITE_TTL_MS) {
        pendingLiveInvites.delete(targetUserId);
        playPrompt(twiml, req, "live_connect_failed.mp3",
          "We were unable to connect your call. Returning you to the male box.");
        twiml.redirect("/voice/browse-profiles");
        ...
        const ringGather = twiml.gather({
          action: `/voice/live-connect-wait?targetUserId=${encodeURIComponent(targetUserId)}&ringCount=${ringCount + 1}`,
          finishOnKey: "#",
          timeout: 1,
        });
        ...
        if (fs.existsSync(ringFileCat2)) {
          ringGather.play(`${baseUrl(req)}/uploads/${cat2}/live_connect_ringing.mp3`);
        } else if (fs.existsSync(ringFileRoot2)) {
          ringGather.play(`${baseUrl(req)}/uploads/live_connect_ringing.mp3`);
        } else {
          ringGather.pause({ length: 10 });
        }
