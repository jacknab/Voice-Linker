...
export async function registerVoiceRoutes(app: Express): Promise<void> {
  ...
  app.post("/voice/cs-account-status", async (req, res) => {
    ...
    try {
      ...
      const greetingLine = profile?.recordingUrl
        ...
        : "You do not have a greeting recorded yet.";
      ...
      dSay(gather,
        `Account details. Status: ${statusLabel}. Membership: ${tier}. ` +
        `Time remaining: ${formatTime(remaining)}. ` +
        `${greetingLine} ${cardLine} ` +
        `Press 2 to add more time. Press 9 to return to customer service. Press star for the main menu.`
      );
    } catch {
      const gather = twiml.gather({ numDigits: 1, finishOnKey: "", action: "/voice/handle-cs-account-status", timeout: 10 });
      dSay(gather, "We were unable to retrieve your account information. Press 9 to return. Press star for the main menu.");
    }
    twiml.redirect("/voice/customer-service");
    res.type("text/xml");
    res.send(twiml.toString());
  ...
  app.post("/voice/cs-billing-info", async (req, res) => {
    ...
    dSay(gather,
      ...
      `Time is deducted while you are connected. You can add more time by pressing 2 from the main menu. ` +
      `If you were recently charged and your time has not been applied, please leave a message for our billing team. ` +
      `Press 2 to add time now. Press 4 to leave a message for the billing team. ` +
  ...
  app.post("/voice/cs-leave-message", async (req, res) => {
    const twiml = new VoiceResponse();
    dSay(twiml, "Please describe your billing question or issue after the tone. Press any key when you are done.");
  ...
  app.post("/voice/cs-save-message", async (req, res) => {
    ...
    dSay(twiml, "Your message has been received. Our billing team will review it and follow up as soon as possible. Thank you for calling.");
    twiml.redirect("/voice/customer-service");
    res.type("text/xml");
    res.send(twiml.toString());
  });
