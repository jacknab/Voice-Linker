...
// Play individual digits of a string (e.g. a mailbox number or membership number)
// using the pre-generated num_N.mp3 files for 0–9.
// e.g. "12345" → num_1.mp3, num_2.mp3, num_3.mp3, num_4.mp3, num_5.mp3
function playDigits(
  twiml: { say: (text: string) => void; play: (url: string) => void },
  req: Request,
  numStr: string
...
// Play "New caller from {region}." using the pre-generated city_{slug}.mp3
// (full announcement at uploads/ root).  Falls back to phrase prefix +
// city_word file and triggers background generation of both files.
...
// (e.g. after "Now playing callers from" or "This caller is from").
// Triggers background generation if the file doesn't exist yet.
...
//   2. callMembershipOverride — they entered a 5-digit membership number + PIN this session
...
export async function registerVoiceRoutes(app: Express): Promise<void> {
  ...
  // This is a safety net — it does NOT replace the webhook; it catches the cases
  // the webhook misses.
  (function startCallStatusPolling() {
    ...
    setInterval(async () => {
      ...
      try {
        ...
        return; // DB unavailable — skip this tick
      }
      ...
      for (const callSid of activeSids) {
        try {
          ...
          // A 404 means Twilio has no record of this call — it's definitely over
          ...
          // Other errors (rate limit, network) — silently skip this SID this tick
        }
      }
    }, POLL_INTERVAL_MS);
  ...
  app.post("/voice", async (req, res) => {
    ...
    if (isAnonymous) {
      ...
      if (anonSiteConf?.blockAnonymousCallers) {
        ...
        const blockTwiml = new VoiceResponse();
        playPrompt(blockTwiml, req, "anon_caller_greeting.mp3",
          "Welcome. We're sorry, but this service requires a visible caller ID.");
        playPrompt(blockTwiml, req, "no_caller_id.mp3",
          "Please disable call blocking and try again. Goodbye.");
        blockTwiml.hangup();
        res.type("text/xml");
            ...
            if (entryUser.accountStatus === "banned") {
              ...
              if (linkedCard && linkedCard.valueSeconds > 0) {
                ...
                if (!callTimeAnnounced.has(entrySid)) {
                  playTimeRemaining(twiml, req, Math.round(linkedCard.valueSeconds / 60));
  ...
  app.post("/voice/handle-anon-entry", async (req, res) => {
    ...
    if (finishedOnKey === "#" && !digits) {
      ...
      playPrompt(twiml, req, "membership_entry_prompt.mp3",
        "Please enter your 5-digit membership card number.");
      twiml.redirect("/voice/anon-entry");
    }
    ...
    try {
      ...
      if (!card || card.valueSeconds <= 0) {
        ...
        if (!callTimeAnnounced.has(callSid)) {
          playTimeRemaining(twiml, req, Math.round(card.valueSeconds / 60));
          callTimeAnnounced.add(callSid);
        }
        const siteConf = await getSiteSettingsCached();
        twiml.redirect(siteConf.siteCategory === "MW" ? "/voice/mw-main-menu" : "/voice/main-menu");
      ...
      if (!isRealPhoneNumber(fromNumber)) {
        ...
        if (cardId || overridePhone) {
          ...
          try {
            ...
            if (cardId) {
              const card = await storage.getMembershipCardById(cardId);
              const minutes = card ? Math.round(card.valueSeconds / 60) : 0;
              ...
              const minutes = Math.round((memberUser.remainingSeconds ?? 0) / 60);
              if (minutes > 0 && !callTimeAnnounced.has(callSid)) {
                playTimeRemaining(twiml, req, minutes);
                callTimeAnnounced.add(callSid);
              }
  ...
  app.post("/voice/voicemail", async (req, res) => {
    ...
    try {
      ...
      if (newCount === 0 && savedCount === 0) {
        ...
        if (newCount > 0) {
          playPrompt(gather, req, "phrase_you_have.mp3", "You have");
          playNumber(gather, req, newCount);
          playPrompt(gather, req, newCount === 1 ? "phrase_new_message.mp3" : "phrase_new_messages.mp3",
            newCount === 1 ? "new message." : "new messages.");
        } else {
          playPrompt(gather, req, "vm_no_new.mp3", "You have no new messages.");
        }
        if (savedCount > 0) {
          playPrompt(gather, req, "phrase_and.mp3", "and");
          playNumber(gather, req, savedCount);
          playPrompt(gather, req, savedCount === 1 ? "phrase_saved_message.mp3" : "phrase_saved_messages.mp3",
            savedCount === 1 ? "saved message." : "saved messages.");
        }
      }
  ...
  app.post("/voice/live-connect-record-invite-done", async (req, res) => {
    ...
    try {
      ...
      // Play the recording back so the caller can hear how it sounds,
      // then ask them to press 1 to send it.
      ...
      // Timeout fallback — return to profiles if no key pressed
      twiml.redirect("/voice/browse-profiles");
    } catch (error) {
  ...
  // ─── 8a-pre. Live Connect: Confirm Invite Before Sending ─────────────────
    ...
    try {
      ...
      if (!invite) {
        ...
        if (fs.existsSync(ringFileCat)) {
          ringGather.play(`${baseUrl(req)}/uploads/${cat}/live_connect_ringing.mp3`);
        } else if (fs.existsSync(ringFileRoot)) {
          ringGather.play(`${baseUrl(req)}/uploads/live_connect_ringing.mp3`);
        } else {
          ringGather.pause({ length: 10 });
        }
        // Fallback redirect in case gather completes without firing
  ...
  app.post("/voice/review-message", async (req, res) => {
    ...
    try {
      ...
      // "Here's how it sounds. Press 1 at any time to send."
      ...
      gather.pause({ length: 1 });
      // "Re-record your message, press 2. To repeat your message, press 3. To cancel your message, press pound."
