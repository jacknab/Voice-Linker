...
- VPS updates should use `bash update.sh` instead of raw `git pull`; the script backs up `.env`, marks it skip-worktree, pulls code, then restores the local `.env` if git touched it.
...
- Live caller incident feed: every `/voice/*` webhook updates the active caller row with `current_ivr_state`, `current_ivr_path`, and `current_ivr_updated_at`. Admin endpoints return this state through `/api/admin/live-callers`; dashboard/live-caller views auto-refresh to show whether each caller is browsing, recording, listening to voicemail, purchasing, etc.
...
- **Seeded caller simulator**: Admin-uploaded seed profiles now activate as a rotating subset (max 3 concurrent admin seed sessions) when a real caller arrives. Each seed session cycles online/offline during its 20–35 minute window instead of keeping every seed online continuously. Startup also marks any stale `VIRTUAL-*` caller rows disconnected before starting fresh simulator sessions.
